...
@section('user-orders')
    @if($count)
        <p class="fs-5 ms-2 pb-3 fw-bold">{{ $count ? 'Всего заказов в пути: ' . $count : ''}}</p>
            ...
            @foreach($orders as $index => $order)
                <div href="#" class="order__row {{ $order->order_status == 2 ? 'opacity-50' : null }} d-flex flex-wrap justify-content-center flex-lg-nowrap align-items-center pt-3 ps-3 pe-3 m-2 text-decoration-none">
                    ...
                    <div class="order__information w-100 h-100">
                        ...
                        <div class="">
                            <a href="{{ route('user.orders.active.detail', $order->id) }}" class="btn btn-primary w-100 btn-lg">Посмотреть детали</a>
                        </div>
                        <p class="text-center fs-6 pt-3">Ваш заказ летит к вам. Нажмите на кнопку, чтобы посмотреть статус.</p>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    @else
        <h3 class="h3 pt-5 pb-5 text-dark text-center">
            На данный момент у вас заказов в пути нет.
        </h3>
    @endif
@endsection
