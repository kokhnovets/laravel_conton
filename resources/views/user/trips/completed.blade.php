...
@section('trips')
    @if($count)
        <p class="fs-5 ms-2 pb-3 fw-bold">{{ $count ? 'Всего доставленных заказов: ' . $count : ''}}</p>
            ...
            @foreach($orders as $index => $order)
                <div href="#" class="order__row {{ $order->order_status == 2 ? 'opacity-50' : null }} d-flex flex-wrap justify-content-center flex-lg-nowrap align-items-center pt-3 ps-3 pe-3 m-2 text-decoration-none">
                    ...
                    <div class="order__information w-100 h-100">
                        ...
                        <p class="order__way-delivery-time mb-2 fs-6">{{ $order->delivery_from }} &#10230; {{ $order->where_to_deliver }}, не позднее {{ Jenssegers\Date\Date::parse($order->deliver_to)->format('j F Y г.') }}</p>
                        <div class="d-inline-flex pe-2 align-items-center">
                            <img class="order__photo_profile_path object-fit-cover" src="{{ $order->user->photo_profile_path ? Storage::url($order->user->photo_profile_path) : URL::asset('images/lk/user.png') }}"  alt="{{ $order->user->first_name }} {{ $order->user->last_name }}">
                            <p class="" style="margin-bottom: 0;">{{ $order->user->first_name }} {{ $order->user->last_name }}</p>
                        </div>
                        ...
                        <div class="">
                            <a href="{{ route('user.trips.completed.detail', $order->id) }}" class="btn btn-primary w-100 btn-lg">Посмотреть детали</a>
                        </div>
                        <p class="text-center fs-6 pt-3">Заказ доставлен.</p>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    @else
        <h3 class="h3 pt-5 pb-5 fw-bold text-center">
            На данный момент у вас нет выполненных заказов.
        </h3>
    @endif
@endsection
